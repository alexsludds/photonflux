...
| [`mrm_tdec_sky130.py`](mrm_tdec_sky130.py) | **53.125 GBd NRZ transmitter co-optimization for OMA − TDEC**, scored by [stateye](https://github.com/AyarLabs/stateye) through `photonflux.tdec`: bus gap (→ κ² via `photonflux.coupler`) × laser lock point × SKY130 inverter (W_p, W_n, L). Full-period PRBS-13 through a 4th-order Bessel–Thomson reference receiver; a PRBS-9 surrogate carries the search (`single`, `sweep`, `optimize`). Needs the optional `eye` extra | — |
...
| [`raman_sbs.py`](raman_sbs.py) | **Raman + Brillouin scattering** (`raman_amp.va`, `sbs_fiber.va`): the two-channel SRS power-transfer slope, the counter-pumped Raman-amp on/off gain vs pump, pump depletion, and the SBS threshold clamp / backward-Stokes growth — all pinned to closed form | — |
