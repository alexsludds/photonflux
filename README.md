...
| `mrm_pam4_tdecq.py` | **53.125 GBd PAM4 microring transmitter scored by TDECQ**: a Gray-coded PRBS-13Q drive steps `ring_mod.va` through four levels into a matched optical termination (`cx.terminator`); stateye (patched DerekK44 fork) scores TDECQ through the BT4 reference receiver, unequalized and through the 802.3dj reference equalizer (15-tap FFE with 3 pre-cursors + 1 DFE tap, `stateye.ffe_mmse`, C_eq into TDECQ). `--rlm` pre-distorts the levels for the ring's Lorentzian. Browser twin: example 46 (Eye tab -> stateye Measure, simulated and equalized eyes side by side). Needs `.[eye]` + the patch | `out/mrm_pam4_tdecq.png` |
...
| `raman_sbs.py` | **Raman + Brillouin scattering** (`models/optical_field/raman_amp.va`, `sbs_fiber.va`): the two-channel SRS power-transfer slope, the counter-pumped Raman-amp on/off gain vs pump, pump depletion, and the SBS threshold clamp with backward-Stokes growth — every scaling pinned to closed form | `out/raman_sbs.png` |
