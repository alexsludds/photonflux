...
| `mrm_pam4_tdecq.py` | **53.125 GBd PAM4 microring transmitter scored by TDECQ**: a Gray-coded PRBS-13Q drive steps `ring_mod.va` through four levels, through an optical attenuator (`cx.attenuator`, `--atten-db`) into a matched `cx.terminator`; stateye (patched DerekK44 fork) scores TDECQ through the BT4 reference receiver, unequalized and through the 5-tap reference FFE (`stateye.ffe_mmse`, C_eq into TDECQ). `--rlm` pre-distorts the levels for the ring's Lorentzian. Browser twin: example 46 (Eye tab -> stateye Measure, simulated and equalized eyes side by side). Needs `.[eye]` + the patch | `out/mrm_pam4_tdecq.png` |
